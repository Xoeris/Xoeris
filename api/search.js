...
export default async function handler(req) {
  ...
  try {
    // 1. Wikipedia Summary / Definition (adds instant high-signal encyclopedic results)
    const wikiPromise = fetchWikipediaResults(query);

    // 2. DuckDuckGo Instant Answer / Topics API (JSON, privacy-preserving)
    const ddgPromise = fetchDdgInstantResults(query);

    // 3. If category is 'code', fetch developer repositories/topics from GitHub public search
    const codePromise = category === "code" ? fetchCodeResults(query) : Promise.resolve([]);

    const [wikiResults, ddgResults, codeResults] = await Promise.allSettled([
      wikiPromise,
      ddgPromise,
      codePromise,
    ]);
    ...
    // 4. Fallback search synthesis if upstream rates are constrained
    if (results.length === 0) {
      results.push(...generateFallbackWebResults(query, category));
    }
  } catch (err) {
...
function generateFallbackWebResults(query, category) {
  ...
  if (category === "code" || qLower.includes("code") || qLower.includes("github") || qLower.includes("ide")) {
    return [
      {
